<div class="<?=$classes;?>">
	<div class="sklad-coordinates">
		<div class="address">
			<?=render($content['field_skladdresinmap']);?>
		</div>
		<div class="mail-phone">
			<?=render($content['field_sklphone']);?>
			<?=render($content['field_sklmail']);?>
		</div>
		<div class="time-job">
			<?=render($content['field_skltimeofjob']);?>
			<?=render($content['field_sklplusstext']);?>
		</div>
	</div>
	<?=render($content['body']);?>
	<?=render($content['field_skltitleproductions']);?>
	<?=render($content['field_sklproduction']);?>
	<div class="info-for-users">Если вы не нашли на нашем сайте нужного материала, но он есть в списке склада — звоните нашим менеджерам, они дадут полную информацию о наличии и цене выбранного товара.</div>
	<div class="rekvisits">
		<div class="title">Реквезиты для оплаты:</div>
		<div class='nalog-data'>
			<div class="wrap">
				...
				<p>Номер расчетного счета - <span><?=token_replace('[mk:rekv:rs]');?></span></p>
